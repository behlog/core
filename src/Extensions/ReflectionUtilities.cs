...
public static class ReflectionUtilities
{
    ...
    public static void AddBehlogClasses(IServiceCollection services, IEnumerable<Assembly> assembliesToScan)
    {
        assembliesToScan = (assembliesToScan as Assembly[] ?? assembliesToScan).Distinct().ToArray();

        var openCommandAndQueryInterfaces = new[]
        {
            typeof(IBehlogQueryHandler<,>),
            typeof(IBehlogCommandHandler<>),
            typeof(IBehlogCommandHandler<,>)
        };

        var openEventInterfaces = new[]
        {
            typeof(IBehlogEventHandler<>),
        };
    ...
    public static IServiceCollection AddBehlogMiddleware(
        IServiceCollection services, IEnumerable<Assembly> assembliesToScan)
    {
        var multiOpenInterfaces = new[]
        {
            typeof(IBehlogMiddleware<,>),
            typeof(IBehlogMiddleware<>)
        };
